...
class SmartLightingCoordinator:
    ...
    def _handle_motion(self, state: State) -> None:
        ...
        # In override states, motion resets timers but not the light [Spec §10]
    ...
    def _handle_single_press(self) -> None:
        ...
        Every physical click toggles the light and restarts the
        temp_override_timeout. The user is in full control.
        Exit ONLY via timeout — no exit by clicking.

        On timeout expiry (_async_temp_override_timeout):
        - Evaluates occupancy + motion
        - Occupancy present: bulbs ON + relay ON
        - No occupancy: bulbs OFF, relay stays ON (if bulb+relay config)
        """
        if self._state != STATE_TEMP_OVERRIDE:
            # Enter temp_override
            ...
            self._cancel_all_timers()
            _LOGGER.info(
                "Entering temp_override, light was %s",
                "ON" if self._is_on else "OFF",
            )

        # Toggle the light (in any temp_override state)
        if self._is_on:
            _LOGGER.info("Temp override: toggling light OFF")
            self._is_on = False
            self._brightness = 0
            self._actuate_off()
        else:
            _LOGGER.info("Temp override: toggling light ON")
            self._is_on = True
            self._brightness = self._get_current_brightness()
            self._actuate_on()

        # Always restart timeout on every click
    ...
    def _start_warning(self) -> None:
        ...
        If warning_dim_duration == 0, skips dim and deactivates immediately.
        Otherwise dims the light to warning_dim_pct % then deactivates
        after warning_dim_duration seconds.
        """
        ...
        if duration <= 0:
            # No warning dim: turn off immediately
            _LOGGER.debug("warning_dim_duration=0, deactivating immediately")
            self._deactivate()
            return
    ...
    async def _async_temp_override_timeout(self) -> None:
        ...
        Re-evaluates occupancy + motion conditions:
        - Occupancy present (or motion if no occupancy sensor):
            bulbs ON + relay ON
        - No occupancy/motion:
            bulbs OFF, relay stays ON (if bulb+relay config)
            only relay → relay OFF
        """
        _LOGGER.info("Temp override timeout: returning to automatic")
        self._temp_override_timer = None
        self._temp_override_timer_end = None
        self._state = STATE_IDLE

        # Determine if area is considered occupied
        has_occupancy_sensor = bool(self.config.get(CONF_OCCUPANCY_SENSOR))
        if has_occupancy_sensor:
            is_present = self._is_occupied()
        else:
            is_present = self._any_motion_active()

        if is_present and self._is_lux_below_threshold():
            # Re-activate normally
            _LOGGER.info("Temp override exit: presence detected, activating")
            self._activate()
        else:
            # No presence: apply correct bulb/relay rule
            _LOGGER.info("Temp override exit: no presence, deactivating")
            bulbs, relay = self._get_active_actuators()
            primary_bulbs = self._to_list(self.config.get(CONF_BULB_ENTITY))
            primary_relay = self.config.get(CONF_RELAY_ENTITY)
            ...
            # Relay: stays ON if bulbs configured, OFF if relay-only
            if primary_relay and not primary_bulbs:
                self.hass.async_create_task(
                    self.hass.services.async_call(
                        "switch", "turn_off", {"entity_id": primary_relay}
            ...
            # else: relay stays ON (power to smart bulbs)
