...
class SmartLightingCard extends HTMLElement {
  ...
  _build() {
    ...
    style.textContent = [
      ...
      /* timer bars section */
      ".tr{width:100%;display:flex;flex-direction:column;gap:4px;margin-bottom:8px}",
      /* active timer row */
      ".tr-active{width:100%;display:none;align-items:center;gap:8px}",
      ".tr-active.v{display:flex}",
      /* failsafe row - always visible when light is on */
      ".tr-failsafe{width:100%;display:none;align-items:center;gap:8px}",
      ".tr-failsafe.v{display:flex}",
      ...
      ".ti{--mdc-icon-size:16px;flex-shrink:0}",
      ".tl{font-size:10px;color:var(--secondary-text-color,#727272);min-width:28px;text-align:right}",
    ...
    /* active timer row (occupancy / temp_override / perm_override) */
    var trActive = document.createElement("div"); trActive.className = "tr-active"; trActive.id = "RA";
    var tiA = document.createElement("ha-icon"); tiA.className = "ti"; tiA.id = "IA";
    trActive.appendChild(tiA);
    var btA = document.createElement("div"); btA.className = "bt";
    var bfA = document.createElement("div"); bfA.className = "bf"; bfA.id = "FA";
    btA.appendChild(bfA); trActive.appendChild(btA);
    var tlA = document.createElement("span"); tlA.className = "tl"; tlA.id = "TLA";
    trActive.appendChild(tlA);
    tr.appendChild(trActive);

    /* failsafe timer row - always shown when light ON */
    var trFs = document.createElement("div"); trFs.className = "tr-failsafe"; trFs.id = "RF";
    var tiF = document.createElement("ha-icon"); tiF.className = "ti"; tiF.id = "IF";
    tiF.setAttribute("icon", "mdi:shield-alert-outline");
    trFs.appendChild(tiF);
    var btF = document.createElement("div"); btF.className = "bt";
    var bfF = document.createElement("div"); bfF.className = "bf"; bfF.id = "FF";
    btF.appendChild(bfF); trFs.appendChild(btF);
    var tlF = document.createElement("span"); tlF.className = "tl"; tlF.id = "TLF";
    trFs.appendChild(tlF);
    tr.appendChild(trFs);
  ...
  _refreshBar() {
    ...
    var RA = this.shadowRoot.getElementById("RA");
    var FA = this.shadowRoot.getElementById("FA");
    var IA = this.shadowRoot.getElementById("IA");
    var TLA = this.shadowRoot.getElementById("TLA");
    if (RA && FA && IA) {
      if (t) {
        RA.classList.add("v");
        ...
        RA.classList.remove("v");
      }
    }

    /* ── Failsafe timer bar (always visible when light on) ── */
    var RF = this.shadowRoot.getElementById("RF");
    var FF = this.shadowRoot.getElementById("FF");
    var IF = this.shadowRoot.getElementById("IF");
    var TLF = this.shadowRoot.getElementById("TLF");
    if (RF && FF) {
      var fsRem = this._num(this._ent.failsafe_timer);
      var fsTot = this._num(this._ent.failsafe_timeout) || 3600;
      if (isOn && fsRem > 0) {
        RF.classList.add("v");
        var fsPct = Math.min(100, (fsRem / fsTot) * 100);
        FF.style.width = fsPct + "%";
        FF.style.background = "#ef5350";
        if (IF) { IF.style.color = "#ef5350"; }
        if (TLF) TLF.textContent = Math.round(fsRem) + "s";
      } else {
        RF.classList.remove("v");
      }
    }
  }
